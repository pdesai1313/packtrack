      ...
      <div className="flex items-end gap-3">
        <div className="flex-1 max-w-xs">
          ...
          <p className="text-gray-400 text-xs mt-1">
            Mismatches within this range show as a warning instead of an error.
          </p>
        </div>
        <button
    ...
    <div className="card">
      ...
      {isLoading ? (
        ...
        <div className="divide-y divide-gray-50">
          {users.map((u) => (
            <div key={u.id} className={`flex items-center justify-between py-2.5 ${!u.active ? 'opacity-50' : ''}`}>
              <div>
                <p className="font-medium text-sm">{u.name}</p>
                <p className="text-gray-400 text-xs">{u.email}</p>
              </div>
              <div className="flex items-center gap-2">
                <span className={`badge ${u.role === 'ADMIN' ? 'badge-red' : u.role === 'REVIEWER' ? 'badge-blue' : 'badge-green'}`}>
                  {u.role}
                </span>
                <button
